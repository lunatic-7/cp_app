      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.titleRow}>
          ...
          <Pressable onPress={toggleDone} style={[styles.doneButton, { backgroundColor: done ? colors.success : colors.surface, borderColor: done ? colors.success : colors.border }]}>
            <Text style={{ fontSize: 20, color: done ? "#fff" : colors.muted }}>{done ? "✓" : "×"}</Text>
            <Text style={{ color: done ? "#fff" : colors.text, fontWeight: "800" }}>{done ? "Done" : "Mark done"}</Text>
          </Pressable>
        </View>
        ...
        {!!problem.hints?.length && <Section title="Hints" colors={colors}>
          {problem.hints.map((hint, index) => <View key={index} style={[styles.hint, { backgroundColor: colors.surfaceAlt }]}>
            <Text style={[styles.hintTitle, { color: colors.text }]}>{hint.title || `Hint ${index + 1}`}</Text>
            <Text style={[styles.body, { color: colors.muted }]}>{hint.content || hint}</Text>
          </View>)}
        </Section>}
...
const styles = StyleSheet.create({
  safe: { flex: 1 }, center: { alignItems: "center", justifyContent: "center" },
  ...
  doneButton: { borderWidth: 1, borderRadius: 16, paddingHorizontal: 13, height: 52, flexDirection: "row", alignItems: "center", gap: 7 },
  ...
  body: { fontSize: 15, lineHeight: 23 }, bullet: { fontSize: 14, lineHeight: 22 }, code: { borderRadius: 13, padding: 14, gap: 8 }, codeText: { fontSize: 13, lineHeight: 20, fontFamily: "monospace" }, codeLabel: { fontWeight: "800", color: "#6366f1" },
  hint: { borderRadius: 13, padding: 14, gap: 5 }, hintTitle: { fontSize: 14, fontWeight: "800" }, notes: { minHeight: 155, borderWidth: 1, borderRadius: 14, padding: 14, fontSize: 15, lineHeight: 22 },
